...
int forwardModel(const SingleGroup *input, SingleGroup *output, SingleGroup *trapPixelMap, CTEParamsFast *ctePars) {
    ...
    if ((trapPixelMap->sci.data.storageOrder != COLUMNMAJOR) || (input->sci.data.storageOrder != COLUMNMAJOR))
        return (status = ALLOCATION_PROBLEM);
    ...
    #ifdef _OPENMP
    #pragma omp parallel \
        shared(input, output, ctePars, cteRprof, cteCprof, trapPixelMap, allocationFail, runtimeFail, status)
    #endif
    {
        int localStatus = HSTCAL_OK;
        //Note: used to set extern int status atomically, note global status takes last set value
        ...
        if (!allocationFail && model) {
            {
                ...
                for (j = 0; j < nColumns; ++j) {
                    ...
                    if ((localStatus = simulateColumnReadout(model, traps, ctePars, cteRprof, cteCprof, nRows,
                                                             ctePars->n_par))) {
                    ...
                    do {
                          ...
                          IF WE'VE CR-RESCALED, THEN IMPLEMENT CTEF*/
                        {
                            ...
                            for (NITINV = 1; NITINV <= ctePars->n_forward - 1; ++NITINV) {
                                ...
                                if ((localStatus = simulateColumnReadout(
                                    model, traps, ctePars, cteRprof, cteCprof, nRows, ctePars->n_par))) {
                                    setAtomicFlag(&runtimeFail);
                                    setAtomicInt(&status, localStatus);
                                    localOK = False;
                                    break;
                        ...
                        REDO = ctePars->fix_rocr
                                   ? correctCROverSubtraction(traps, model, observed, nRows, ctePars->thresh)
                                   : False;
...
int simulatePixelReadout_v1_1(double *const pixelColumn, const float *const traps, const CTEParamsFast *const ctePars,
    ...
    //go up or down? (if swap, change below condition)
    for (int w = maxChargeTrapIndex; w >= 0; --w) {
        //is any of this even needed or can we just directly map?
            ...
            /*GO UP THE COLUMN PIXEL BY PIXEL*/
            {
                ...
                for (i = 0; i < nRows; ++i) {
                    ...
                    if (pixel >= ctePars->qlevq_data[w]) {
                        ...
                        //ttrap-1 may not be the same index as ref'd in rprof???
    ...
    */
    ...
    Bool redo = False;
    {
        ...
        for (i = 10; i < nRows - 2; ++i) {
            if (((threshHold > pix_model[i]) && (threshHold > (pix_model[i] - pix_observed[i])))
                ...
                || (((pix_model[i] + pix_model[i + 1] + pix_model[i + 2]) < -15.)
                    && ((pix_model[i] + pix_model[i + 1] + pix_model[i + 2] - pix_observed[i] - pix_observed[i + 1] - pix_observed[i + 2]) < -15.))) {
                redo = True;
                unsigned iMax = i;
...
int populateTrapPixelMap(SingleGroup *trapPixelMap, CTEParamsFast *ctePars) {
    ...
    */
    ...
    if (ctePars->verbose) {
        ...
        trlmessage("(pctecorr) Time taken to populate pixel trap map image: %.2f(s) with %i threads",
...
*/
                ...
                nrmsLocal = 0;
                {
                    ...
                    for (i = 0; i < nColumns; ++i) {
...

                        {
                            unsigned j;
                            for (j = 0; j < nRows; ++j)
                                PixColumnMajor(adjustment.sci.data, j, i) = find_dadjFast(
                                    1 + i - imid, j, nRows, obs_loc, rsz_loc, ampReadNoise);
                        }
                    }
                    ...
                    for (i = 0; i < nColumns; ++i) {
                        {
                            unsigned j;
                            for (j = 0; j < nRows; ++j) {
                                PixColumnMajor(output->sci.data, j, i) += (PixColumnMajor(adjustment.sci.data, j, i) *
                                    0.75);
                                PixColumnMajor(readNoise.sci.data, j, i) = (PixColumnMajor(input->sci.data, j, i) -
                                    PixColumnMajor(output->sci.data, j, i));
                            }
                        }
                    }
                    ...
                    for (j = 0; j < nColumns; ++j) {
                        {
                            unsigned i;
                            for (i = 0; i < nRows; ++i) {
                                if ((fabs(PixColumnMajor(input->sci.data, i, j)) > 0.1 || fabs(
                                    PixColumnMajor(output->sci.data, i, j)) > 0.1)) {
    ...
    if (ctePars->verbose) {
        double timeSpent = ((double)(clock() - begin)) / CLOCKS_PER_SEC;
        trlmessage("(pctecorr) Time taken to smooth image: %.2f(s) with %i threads", timeSpent / ctePars->maxThreads,
                   ctePars->maxThreads);
    }
...
double find_dadjFast(const unsigned i, const unsigned j, const unsigned nRows, const float *obsloc[3],
                     const float *rszloc[3], const double readNoiseAmp
) {
    /*
