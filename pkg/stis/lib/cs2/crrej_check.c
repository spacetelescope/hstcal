	...
	*nimgs = 0;
	...
	for (n = 0; n < c_imtlen(tpin); ++n) {
		...
		/* also read the keyword OBSTYPE */
		    ...
		    trlerror("Keyword OBSTYPE not found in file %s.\n", fdata);
		    return (2);
		}
	    }
		...
		trlerror("Keyword READNSE not found in file %s.\n", fdata);
		return (2);
	    }
	    if (getKeyF (&prihdr, "ATODGAIN", &gn) != 0) {
		trlerror("Keyword ATODGAIN not found in file %s.\n", fdata);
		return (2);
	    }
	    if (gn == 0.) {
		trlerror("Keyword ATODGAIN in file %s is 0.\n", fdata);
		return (2);
	    }
		...
		if (strcmp (ccdamp0, ccdamp) != 0) {
		    trlerror("%s uses different CCDAMP.\n", fdata);
		    return (2);
		}
	    }
	...
	for (k = 0; k < *nimgs; ++k) {
		...
		*dim_y = getNaxis2(ipsci[k]);
		...
		trlerror("file '%s[%d][sci]' does not have the same size as the first image\n", imgname[k], grp[k]);
		return (2);
	    }
	}
