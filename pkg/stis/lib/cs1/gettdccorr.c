...
*/
	...
	if (c_iraferr()) {
	    trlerror("ERROR    Can't open TDCTAB `%s'.\n", sts->tdctab.name);
	    return OPEN_FAILED;
	}
	...
	if (tbl_type == POST_SM4_TDC_FORMAT) {
	    ...
	    trlerror("ERROR    TDCTAB `%s' is not a recognized format.\n",
		sts->tdctab.name);
	    return OPEN_FAILED;
	}
...
*/
	...
	if (nrows < 1) {
	    trlerror("ERROR    TDCTAB %s is empty.\n", sts->tdctab.name);
	    c_tbtclo(tp);
	    return TABLE_ERROR;
	}
	...
	if (cp_date0 == 0 || cp_temp0 == 0 ||
	    ...
	    trlerror("ERROR    Column not found in TDCTAB.\n");
	    c_tbtclo(tp);
	...
	if (sts->expstart < min_date0) {
	    trlerror("ERROR    Exposure start time precedes earliest date "
	           "in TDCTAB.\n");
	    c_tbtclo(tp);
	    return TABLE_ERROR;
	}
	...
	*/
	...
	if (count_rate > 100.) {
	    trlwarn("Warning  TDC dark count rate = %.6g\n", count_rate);
	}
	*factor = count_rate / mean_dark;
	if (fabs(*factor) > 100. || fabs(*factor) < 0.01) {
	    trlwarn("Warning  TDC correction factor = %.6g.\n", *factor);
	}
...
*/
	...
	for (row = 1; row <= tabinfo.nrows; row++) {
		...
		trlwarn("Warning  Row %d of TDCTAB is DUMMY.\n", row);
...
static int OpenTdcTab (char *tname, TblInfo *tabinfo) {
	...
	if (c_iraferr()) {
	    trlerror("ERROR    TDCTAB `%s' not found.\n", tname);
	    return OPEN_FAILED;
	}
