...
*/
	...
	*/
	...
	if (count_rate > 100.) {
	    trlwarn("TDC dark count rate = %.6g\n", count_rate);
	}
	*factor = count_rate / mean_dark;
	if (fabs(*factor) > 100. || fabs(*factor) < 0.01) {
	    trlwarn("TDC correction factor = %.6g.\n", *factor);
	}
...
*/
	...
	for (row = 1; row <= tabinfo.nrows; row++) {
		...
		trlwarn("Row %d of TDCTAB is DUMMY.\n", row);
