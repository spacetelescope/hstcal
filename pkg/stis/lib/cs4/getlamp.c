...
*/
	...
	for (row = 1;  row <= tabinfo.nrows;  row++) {
		...
		if (sts->lamptab.goodPedigree == DUMMY_PEDIGREE) {
		    ...
		    trlwarn("Warning  LAMPTAB has PEDIGREE = DUMMY.\n");
	...
	if (!foundit) {
	    trlerror("ERROR    LAMP %s, LAMPSET %s not found in %s.\n",
...
static int OpenLampTab (char *tname, TblInfo *tabinfo) {
	...
	if (c_iraferr()) {
	    trlerror("ERROR    LAMPTAB `%s' not found.\n", tname);
	    return (OPEN_FAILED);
	}
	...
	if (tabinfo->cp_lampset == 0 ||
	    ...
	    trlerror("ERROR    Column not found in LAMPTAB.\n");
	    c_tbtclo (tabinfo->tp);
...
static int ReadLampArray (TblInfo *tabinfo, int row, LampInfo *lamp) {
	...
	if (nwl < lamp->nelem || nflux < lamp->nelem) {
	    c_tbtclo (tabinfo->tp);
	    trlerror("ERROR    Not all elements were read from LAMPTAB.\n");
	    return (TABLE_ERROR);
	}
