...
void PrBegin (char *label) {
	...
	trlmessage ("\n");
	snprintf(MsgText, sizeof(MsgText), "%s*** %s -- Version %s ***",TRL_PREFIX, label, ACS_CAL_VER);
	trlmessage (MsgText);
...
void PrHdrInfo (char *aperture, char *filter1, char *filter2, char *detector) {
	...
	trlmessage (MsgText);
	snprintf(MsgText, sizeof(MsgText), "FILTER1 %s", filter1);
	trlmessage (MsgText);
	snprintf(MsgText, sizeof(MsgText), "FILTER2 %s", filter2);
	trlmessage (MsgText);
