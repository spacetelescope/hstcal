...
*/
    ...
    if (update == YES) {
        ...
        hstio_status = hstio_err();
        if (status || hstio_status) {
            retStatus = (hstio_status != HSTOK) ? hstio_status : status;
            return (retStatus);
        }
        ...
        if (BinCoords (&a->sci.hdr, block, offset, &b->sci.hdr, &b->err.hdr, &b->dq.hdr)) {
            return (status);
        }
    }
