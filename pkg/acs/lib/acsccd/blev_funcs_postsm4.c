 ...
 */
int bias_shift_corr(ACSInfoRef *acs, int nGroups, ...) {

  /* This function has built in assumptions regarding the data being passed */
  assert (acs != NULL);  /* Make sure the ACSInfoRef structure is defined */
...
void cross_talk_corr(ACSInfoRef *acs, SingleGroup *im) {
  /* iteration variables */
  int i, j;
...
int doDestripe(ACSInfoRef *acs, SingleGroup *chip2, SingleGroup *chip1) {
  extern int status;
