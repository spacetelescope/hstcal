...
int PixCteParams (char *filename, const double expstart, ACSCTEParams *pars) {
  ...
  if (c_iraferr() || col_ptr_dtde == 0) {
    snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error getting column %s of PCTETAB",dtde);
    trlerror(MsgText);
  ...
  if (c_iraferr() || col_ptr_qdtde == 0) {
    snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error getting column %s of PCTETAB",qdtde);
    trlerror(MsgText);
    status = COLUMN_NOT_FOUND;
    return status;
  }
  ...
  for (j = 0; j < NUM_PHI; j++) {
    ...
    if (c_iraferr()) {
      snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error reading row %d of column %s in PCTETAB",j+1, dtde);
      trlerror(MsgText);
      status = TABLE_ERROR;
      return status;
    }
    ...
    if (c_iraferr()) {
      snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error reading row %d of column %s in PCTETAB",j+1, qdtde);
      trlerror(MsgText);
      status = TABLE_ERROR;
      return status;
    }
  ...
  if (c_iraferr() || col_ptr_levs == 0) {
    snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error getting column %s of PCTETAB",level);
    trlerror(MsgText);
    status = COLUMN_NOT_FOUND;
    return status;
  }
  ...
  for (j = 0; j < NUM_LEV; j++) {
    ...
    if (c_iraferr()) {
      snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error reading row %d of column %s in PCTETAB",j+1, level);
      trlerror(MsgText);
      status = TABLE_ERROR;
      return status;
    }
  ...
  if (c_iraferr() || col_ptr_psin == 0) {
    snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error getting column %s of PCTETAB",mjdstr);
    trlerror(MsgText);
    status = COLUMN_NOT_FOUND;
    return status;
  }
  ...
  if (c_iraferr() || col_ptr_logq == 0) {
    snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error getting column %s of PCTETAB",scalestr);
    trlerror(MsgText);
    status = COLUMN_NOT_FOUND;
    return status;
  }
  ...
  for (j = 0; j < NUM_SCALE; j++) {
    ...
    if (c_iraferr()) {
      snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error reading row %d of column %s in PCTETAB",j+1, mjdstr);
      trlerror(MsgText);
      status = TABLE_ERROR;
      return status;
    }
    ...
    if (c_iraferr()) {
      snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error reading row %d of column %s in PCTETAB",
              j+1, scalestr);
      trlerror(MsgText);
      status = TABLE_ERROR;
      return status;
  ...
  for (j = 0; j < AMP_COLS; j++) {
    ...
    for (k = 0; k < NAMPS; k++) {
      ...
      if (c_iraferr() || col_ptr_amp == 0) {
        snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error getting column %s of PCTETAB",amp_keys[k]);
        trlerror(MsgText);
        status = COLUMN_NOT_FOUND;
        return status;
      }
  ...
  for (l = 0; l < nchg_leak; l++) {
    ...
    if (chg_mjd1 <= expstart && chg_mjd2 > expstart) {
      ...
      if (c_iraferr() || col_ptr_psin == 0) {
        snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error getting column %s of PCTETAB",node);
        trlerror(MsgText);
        status = COLUMN_NOT_FOUND;
        return status;
      }
      ...
      for (j = 0; j < NUM_PSI; j++) {
        ...
        if (c_iraferr()) {
          snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error reading row %d of column %s in PCTETAB",j+1, node);
          trlerror(MsgText);
          status = TABLE_ERROR;
          return status;
        }
        ...
        for (k = 0; k < NUM_LOGQ; k++) {
          ...
          if (c_iraferr() || col_ptr_logq == 0) {
            snprintf(MsgText, sizeof(MsgText), "(pctecorr) Error getting column %s of PCTETAB",logq_keys[k]);
            trlerror(MsgText);
            status = COLUMN_NOT_FOUND;
            return status;
          }
