...
int checkACSKeys(ACSInfo *acs)
{
    ...
    {
        trlerror("DETECTOR = %s is invalid", acs->det);
        return HEADER_PROBLEM;
    }

    if (acs->exptime < 0.)
    {
        trlerror("Exposure time is invalid:  %14.6g.", acs->exptime);
    ...
    if (acs->nimsets < 1)
    {
        trlerror("NEXTEND = %d; must be at least %d.", acs->nextend, EXT_PER_GROUP);
    ...
    if (acs->detector != MAMA_DETECTOR)
    {
        ...
        {
            trlerror("CCDAMP = `%s' is invalid. Must be in 'ABCD'", acs->ccdamp);
            return INVALID_VALUE;
        }
    }
    return HSTCAL_OK;
