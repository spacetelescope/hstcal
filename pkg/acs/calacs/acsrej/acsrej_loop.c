                    ...
                    */
                    if (strncmp(par->initgues,"minimum",3) == 0 && iter == 0) {
                        ...
                        for (i = 0; i < ampx; i++) {
                            /* APPLY SHADCORR correction here, as necessary
                               ...
                               not performed.
                            */
                            dum = PPix(ave,i,line)*efacn/shadcorr[i] +
                                  skyvaln / gn[0];

                            /* clip the data at zero */
                            val = (dum > 0.) ? dum : 0.;

                            /* compute sky subtracted pixel value for use
                               with SCALENSE */
                            pixsky = (dum - skyvaln / gn[0] > 0.) ?
                                     dum - skyvaln / gn[0] : 0.;

                            /* Apply noise and gain appropriate for AMP used
                               for this pixel */
                            thresh[n][width][i] = sig2 *
                                (nse[0] + val/gn[0] +  SQ(scale * pixsky)) /
                                exp2n;
                        ...
                        for (i = ampx; i < dim_x; i++) {
                            ...
                            /* compute sky subtracted pixel value for use
                               with SCALENSE */
                            pixsky = (dum - skyvaln / gn[1] > 0.) ?
                                     dum - skyvaln / gn[1] : 0.;

                            /* Apply noise and gain appropriate for AMP used
                               for this pixel */
                            thresh[n][width][i] = sig2 *
                                (nse[1] + val/gn[1] + SQ(scale * pixsky)) /
                                exp2n;
                    ...
                    if (iter == (niter-1)) {
                        ...
                        /* accumulate the variance only during the
                           last iteration and ONLY for non-HIT or non-SPILL
                           pixels. */
                        for (i = 0; i < ampx; i++) {
                            if ( (mask[n][width][i] & maskdq) == OK) {
