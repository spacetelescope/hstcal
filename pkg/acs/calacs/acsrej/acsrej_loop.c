 ...
 */
static int  initShad (Hdr *, int, char *, int, int *, int *, int *, int *, int *);
static void getShadLine (float **, int, int, int, float *);
static void getShadBuff (IODescPtr *, int, int, int, int, int , int , int , int , float **);
...
/*  crrej_loop -- Perform cosmic ray rejection
 ...
 24-Sept-2001  W.J. Hack       Made allocation of zl in getShadBuff more robust
 */
                 ...
                 ShortTwoDArray *dq, int *nrej, char *shadfile)
{
  ...
  for (iter = 0; iter < niter; iter++) {
    ...
    for (line =0; line < dim_y; line++) { 
      ...
      if (line > 0) {								
        /* Scroll buffers so new line can be inserted into middle row of buffer
         add BLANK line to bottom of buffer 
         */
        bufftop = line+width;
        for (k = 0; k < nimgs; k++) {
          ...
          scrollShortBuff (bufdq, line, dim_y, buffheight, dim_x, mask[k], zerosbuf);
          scrollFloatBuff (buf, line, dim_y, buffheight, dim_x, pic[k], zerofbuf);                   
          scrollFloatBuff (buf, line, dim_y, buffheight, dim_x, thresh[k], zerofbuf);
          scrollFloatBuff (buf, line, dim_y, buffheight, dim_x, spthresh[k], zerofbuf);
        }
      } else {
      ...
      for (n = 0; n < nimgs; n++) {
        ...
        if (efacn > 0.0) {
           ...
           */
          getShadcorr (shadbuff, line, shad_dimy, dim_x, efacn, shadf_x, shadcorr);
          ...
          if (strncmp(par->initgues, "minimum", 3) == 0 && iter == 0) {
            ...
            for (i = 0; i < ampx; i++) {
              ...
              thresh[n][width][i] = sig2 * ((nse[0] + val +  SQ(scale * pixsky))) / exp2n;
              /* Compute threshhold without SCALENSE for use with SPILL pixels*/
              spthresh[n][width][i] = sig2 * ((nse[0] + val)) / exp2n;
            ...
            for (i = ampx; i < dim_x; i++) {
              ...
              /* Apply noise and gain appropriate for AMP used for this pixel */
              thresh[n][width][i] = sig2 * ((nse[1] + val + SQ(scale * pixsky))) / exp2n;
              /* Compute threshhold without SCALENSE for use with SPILL pixels*/
              spthresh[n][width][i] = sig2 * ((nse[1] + val )) / exp2n;
            ...
            if (SQ(pic[n][width][i]-PPix(ave,i,line)) > 
              ...
              for (jj = 0; jj < buffheight; jj++) {
                ...
                for (ii = i-width; ii <= i+width; ii++) {
                  ...
                  if (SQ(pic[n][jj][ii]-PPix(ave,ii,jndx)) <= psig2*spthresh[n][jj][ii])
                    continue;
                  if (mask[n][jj][ii] != HIT)
                    mask[n][jj][ii] = SPILL;
                }
          ...
          if (iter == (niter-1)) {
            ...
            /* accumulate the variance only during the
             last iteration and ONLY for non-HIT or non-SPILL pixels.
             */
            for (i = 0; i < ampx; i++){
              ...
              if ( (mask[n][width][i] & maskdq) == OK) {
                ...
                /* clip the data at zero */
                val = (dum > 0.) ? dum : 0.;
                /* Removed SCALENSE from error. 24-Nov-2003 WJH.                                
            ...
            for (i = 0; i < dim_x; i++) {
              ...
              /* output DQF is just the logical OR of all 
               (original) input DQF */
              bufdq[i] = bufdq[i] & nocr;  /* Added 17 June 99, WJH*/
              ...
              if (mask[n][width][i]==HIT || mask[n][width][i] == SPILL) {
...
static int initShad (Hdr *scihdr, int dimx, char *shadname, int chipext, int *refx,
...
static void getShadLine (float **shad, int line, int nlines, int dimx, float *shadline) {
...
static void getShadcorr (float **shad, int line, int nlines, int dimx, float efacn, int
                         shadf_x, float *shadline) {
...
static void getShadBuff (IODescPtr *ipshad, int line, int shad_dimy, int dim_x, int shad_x, int rx, int ry, int x0, int y0, float **shad) {
...
/* 
 ...
 lines of data from the image.
 */
static void InitShortSect (short **sect, short *sbuf, IODescPtr *ipdat, int line, int width, int dimx){
  /* Parameters:
