...
int acsrej_do (IRAFPointer tpin, char *outfile, char *mtype, clpar *par,
    ...
    char        root[ACS_FNAME];    /* ROOTNAME for output CRJ/CRC file */
    ...
    int         found;
    char        imgdefault[ACS_FNAME];  /* 1st input img name with EXPTIME>0 */
    ...
    if (par->shadcorr == PERFORM || shadswitch == PERFORM) {
        ...
        if (GetKeyStr (&phdr, "SHADFILE", NO_DEFAULT, "", shadrefname,
                       ACS_FNAME))
            return(status);
