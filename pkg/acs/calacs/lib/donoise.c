	...
	*/
    ...
    if (acs->detector != MAMA_DETECTOR){ 
		...
		*/
        ...
        /* For WFC data, AMPY will always be zero, yet we still need
            to select the correct ccdbias value to apply.  Determine
            that here.
        */
        if (acs->detector == WFC_CCD_DETECTOR && acs->chip == 2){
            ccdbias[0] = acs->ccdbias[2];
            ccdbias[1] = acs->ccdbias[3];
            ccdbias[2] = acs->ccdbias[2];
            ccdbias[3] = acs->ccdbias[3];
        } else {
            for (i=0;i<NAMPS;i++) ccdbias[i] = acs->ccdbias[i];
        }
		    ...
		    */
	        ...
	        for (i = 0;  i < ampx;  i++) {
                if (use_bias) {
		            ...
		            /* subtract bias level and bias image then convert to electrons */
		            value = (Pix (x->sci.data, i, j) - Pix(b.sci.data, i+x0, j+y0) - bias) * gain[AMP_C];
                } else {
		    ...
		    */
	        ...
	        for (i = ampx;  i < dimx;  i++) {
                if (use_bias) {
                    /* Post-SM4 case: new algorithm */
		            /* subtract bias level and bias image then convert to electrons */
		            value = (Pix (x->sci.data, i, j) - Pix(b.sci.data, i+x0, j+y0) - bias) * gain[AMP_D];
                } else {
                    /* Pre-SM4 case: no change */
		            /* subtract bias and convert to electrons */
		    ...
		    */
	        ...
	        for (i = 0;  i < ampx;  i++) {
                if (use_bias) {
                    /* Post-SM4 case: new algorithm */
		            /* subtract bias level and bias image then convert to electrons */
		            value = (Pix (x->sci.data, i, j) - Pix(b.sci.data, i+x0, j+y0) - bias) * gain[AMP_A];
                } else {
                    /* Pre-SM4 case: no change */
		            /* subtract bias and convert to electrons */
            ...
            */
	        ...
	        for (i = ampx;  i < dimx;  i++) {
                ...
                if (use_bias) {
                    /* Post-SM4 case: new algorithm */
		            /* subtract bias level and bias image then convert to electrons */
		            value = (Pix (x->sci.data, i, j) - Pix(b.sci.data, i+x0, j+y0) - bias) * gain[AMP_B];
                } else {
                    /* Pre-SM4 case: no change */
		            /* subtract bias and convert to electrons */
