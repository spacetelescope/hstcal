...
static int divFlat (SingleGroup *, char *, ACSInfo *, int);
...
*/
	...
	if (acs2d->pfltcorr == PERFORM) {
        if (divFlat (x, acs2d->pflt.name, acs2d, applygain)){
...
static int divFlat (SingleGroup *x, char *flatname, ACSInfo *acs2d, int applygain) {
    ...
    */
    if (ysame_size) {
        for (line = 0; line < scilines; line++) {
            ...
            if (applygain){
	            multgn1d (&y, line, acs2d->ampx, acs2d->ampy, gain, gnscale);
            }
		    /* Now, apply flat-field */
		    div1d (x, line, &y);
        ...
        */
        ...
        for (i=0,line=y_y0; i < scilines; i++,line++) {
            ...
            if (applygain){
	            multgn1d (&ytrim, line, acs2d->ampx, acs2d->ampy, gain, gnscale);
            }
