...
*/
	...
	if (acs2d->photcorr == PERFORM) {
	    ...
	    if (GetTabRef (acs2d->refnames, phdr,
			"GRAPHTAB", &acs2d->graph, &acs2d->photcorr))
		return (status);
	    if (acs2d->graph.exists != EXISTS_YES) {
		    MissingFile ("GRAPHTAB", acs2d->graph.name, missing);
	    }

	    /* Get the name of the components table. */
	    if (GetTabRef (acs2d->refnames, phdr,
			"COMPTAB", &acs2d->comp, &acs2d->photcorr))
		return (status);
	    if (acs2d->comp.exists != EXISTS_YES) {
		/* Only print an error message if a name was specified. */
		if (GotFileName (acs2d->comp.name))
		    MissingFile ("COMPTAB", acs2d->comp.name, missing);
	    }
