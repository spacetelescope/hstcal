...
static void PhotMsg (ACSInfo *acs2d) {
	...
	if (!OmitStep (acs2d->photcorr)) {
	    ...
	    PrRefInfo ("graphtab", acs2d->graph.name, acs2d->graph.pedigree,
			acs2d->graph.descrip, acs2d->graph.descrip2);

		PrRefInfo ("comptab", acs2d->comp.name, acs2d->comp.pedigree,
			acs2d->comp.descrip, acs2d->comp.descrip2);
	}
}
