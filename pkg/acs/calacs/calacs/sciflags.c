...
*/
	...
	if (sci_sw->photcorr == PERFORM) {
	    acs->sci_basic_2d = PERFORM;
	    if (GetNewRef (phdr, "GRAPHTAB", sciref))
		return (status);
	    if (GetNewRef (phdr, "COMPTAB", sciref))
		return (status);
	}
...
static void MAMASanity (int detector, char *calswitch) {
	...
	if (detector != MAMA_DETECTOR) {
	  	sprintf (MsgText, "%s = PERFORM, but detector is CCD.",
			calswitch);
		trlwarn (MsgText);
	}
}
...
static void CCDSanity (int detector, char *calswitch) {
	...
	if (detector == MAMA_DETECTOR) {
	    sprintf (MsgText, "%s = PERFORM, but detector is MAMA.",
			calswitch);
		trlwarn (MsgText);
	}
}
