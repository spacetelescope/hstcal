...
int ACSccd (char *input, char *output, CalSwitch *ccd_sw, RefFileInfo *refnames, int printtime, int verbose) {
   ...
   * for an image that's also getting CTE corrections.
   *
   ...
   * I'm going to set acs.pctecorr back to whatever it was in ccd_sw->pctecorr.
   * MRD 15 Mar 2011
   */
...
static int BiasKeywords (ACSInfo *acs) {
  ...
  if (key != NotFound) {
    /* 
     Update all bias value keywords for all chips to provide uniform sets of
     keywords acrossl all the extensions in multi-chip/multi-extension data.
     Only write out non-zero values to avoid overwriting any previously
     written BIASLEV keyword values. WJH 18-June-2003
     */
    if (acs->blev[0] != 0.) {
      if (PutKeyFlt (&phdr, "BIASLEVA", acs->blev[0], "mean of amp's bias level")) {
        freeHdr (&phdr);
        closeImage (im);
        return (status);
      }
    ...
    if (acs->blev[1] != 0.) {
      if (PutKeyFlt (&phdr, "BIASLEVB", acs->blev[1],	"mean of amp's bias level")) {
        freeHdr (&phdr);
        closeImage (im);
        return (status);
      }
    ...
    if (acs->blev[2] != 0.) {
      if (PutKeyFlt (&phdr, "BIASLEVC", acs->blev[2],	"mean of amp's bias level")) {
        freeHdr (&phdr);
        closeImage (im);
        return (status);
      }
    ...
    if (acs->blev[3] != 0.) {
      if (PutKeyFlt (&phdr, "BIASLEVD", acs->blev[3],	"mean of amp's bias level")) {
        freeHdr (&phdr);
        closeImage (im);
        return (status);
      }
