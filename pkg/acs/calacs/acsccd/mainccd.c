...
int main (int argc, char **argv) {
    ...
    char isuffix[] = "_raw";
    char osuffix[] = "_blv_tmp"; /* _blc_tmp for PCTECORR */
    ...
    for (i = 1;  i < argc;  i++) {
        ...
        if (strcmp (argv[i], "-dqi") == 0) {
            ...
            switch_on = 1;
        } else if (strcmp (argv[i], "-pcte") == 0) {
            ccd_sw.pctecorr = PERFORM;
            switch_on = 1;
        } else if (argv[i][0] == '-') {
        **********/
        if (argv[i][0] == '-') {
    ...
    if (inlist[0] == '\0' || too_many) {
        ...
        printf ("       -dqi -atod -blev -bias -pcte\n");
        */
        FreeNames (inlist, outlist, input, output);
        exit (ERROR_RETURN);
    }
    ...
    /* Was no calibration switch specified on command line?
       default values (mostly PERFORM) except ATOCORR
    if (!switch_on) {*/
    ...
    for (n = 0;  n < n_in;  n++) {
        ...
        /* Determine osuffix based on PCTECORR in header. */
