...
int DoCCD (ACSInfo *acs_info) {
  ...
  for (i = 0; i < acs_info->nimsets; i++) {
    if (FindOverscan(&acs[i], x[i].sci.data.nx, x[i].sci.data.ny, &overscan[i])) {
      freeSingleGroup(&x[i]);
      return (status);
    }
  }
  ...
  if (acs_info->blevcorr == PERFORM && 
    ...
    for (i = 0; i < acs_info->nimsets; i++) {
      ...
      if (done) {
        ...
        sprintf(MsgText, "     mean of bias levels subtracted was %.6g electrons.",
                 meanblev);
        trlmessage(MsgText);
    ...
    if (done) {
      ...
      for (i=0; i < NAMPS-1; i++) {
        if (acs[0].ccdbias[i] > 0) { 
          sprintf(buff, "%.5g,",acs[0].ccdbias[i] * acs[0].atodgain[i]);
          strcat(MsgText, buff);
        }
      }
      
      if (acs[0].ccdbias[NAMPS-1] > 0) { 
        sprintf(buff, "%.5g",acs[0].ccdbias[NAMPS-1] * acs[0].atodgain[NAMPS-1]);
        strcat(MsgText, buff);
      }
      trlmessage(MsgText);
  ...
  for (i = 0; i < acs_info->nimsets; i++) {
    if (blevcorr[i] == COMPLETE || acs[i].blevcorr == COMPLETE) {
      ...
      if (acs_info->verbose) {
        sprintf(MsgText,"Writing out image with trimx = %d,%d, trimy = %d,%d",
                acs[i].trimx[0],acs[i].trimx[1], acs[i].trimy[0],acs[i].trimy[1]);
        trlmessage(MsgText);
