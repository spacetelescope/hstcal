        ...
        ** temporary image */
        if (wf3->pfltcorr == PERFORM || wf3->dfltcorr == PERFORM) {
            ...
            ** in flat */
            ...
            allocSingleNicmosGroup (&dflt, flat->sci.data.nx, flat->sci.data.ny);
            if (hstio_err()) {
...
*/
    ...
    /* Local variables */
    int i, j;            /* loop index */
    char kword[SZ_KEYWORD + 1];    /* keyword name */
    ...
    /* Read the list of exposure times.
       Because of FITS keyword length limitation, ndarks cannot be over 99;
       This check is needed to avoid compilation warning when
       -DENABLE_ASAN=ON is given to cmake. */
    for (i=0, j=1; i < wf3->ndarks && j < 100; i++, j++) {
        sprintf (kword, "EXPOS_%d", j);
        wf3->dtimes[i] = 0;
    ...
    if (strncmp (wf3->sampseq, "NONE", 4) != 0) {
        ...
        ** science data, or two times that bracket it */
        ...
        for (i = 0; i < wf3->ndarks; i++) {
            ...
            if (wf3->dtimes[i] < wf3->exptime[ngroup-1]) {
                wf3->DarkType = INTERP;
                wf3->darkframe1 = i+1;
...
/*  crrpar_in -- Read parameters either from user input or table.
  ...
  Description:
  ------------
  Reads CL parameters and does necessary checks
...
int crrpar_in (clpar *par, int newpar[], int nimgs, float exptot, int *niter,
    ...
    if (newpar[0] < MAX_PAR) {
        ...
        if (crsplit_in >= maxcrsplit)
            crsplit_in = maxcrsplit;
