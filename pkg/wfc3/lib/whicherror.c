...
void WhichError (int status) {
	...
	if (hstio_err())
	    snprintf(MsgText, sizeof(MsgText), "         HSTIO error %d:  %s", status,
		     hstio_errmsg());
	else if (c_iraferr())
	    snprintf(MsgText, sizeof(MsgText), "         IRAF error %d:  %s", c_iraferr(),
		     c_iraferrmsg());
	trlerror (MsgText);
	...
	if (status == OUT_OF_MEMORY) {
	    ...
	    trlerror("Calibration file(s) missing or invalid.");
