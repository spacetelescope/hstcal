...
/* This routine resets the sums and coefficients to zero, and it copies
   ...
   Howard Bushouse, 2000 Aug 29:
	Unchanged ACS code used for WFC3.

   H.Bushouse, 2003 Oct 24:
	Added BlevResults routine for reporting slope and intercept of fit
	(CALACS changes).

   H.Bushouse, 2008 Sep 17:
	Modified fit report in BlevFit to indicate that this is fit to 
	serial overscan.
*/
...
int BlevFit (void) {
	...
	snprintf(MsgText, sizeof(MsgText), "Computed a serial fit with slope of %g and intercept of %g",
		 slope, intercept);
	trlmessage (MsgText);
