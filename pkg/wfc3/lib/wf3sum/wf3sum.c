...
*/
    ...
    if (nrptexp != wf3->nimages) {
        trlwarn("%d exposures will be summed, however, NRPTEXP was %d.", wf3->nimages, nrptexp);
    }
...
static int SumGrps (Wf3SumInfo *wf3, char *mtype) {
    ...
    for (extver = 1;  extver <= wf3->nimsets;  extver++) {
         ...
         for (i = 1; i < wf3->nimages; i++) {
             ...
             ** for the remainder of the images.  */
             ...
             if (GetKeyDbl (y.globalhdr, "EXPTIME", NO_DEFAULT, 0., &exptime))
                 return (status);
             if (GetKeyDbl (y.globalhdr, "EXPEND", NO_DEFAULT, 0., &wf3->expend))
                 return (status);
         ...
         if (PutSumHdrInfo (&x, sumexptime, wf3->expend, wf3->nimages, wf3->nimsets))
             return (status);
...
static void InitSumTrl (char *input, char *output) {
    ...
    size_t trl_len = CHAR_LINE_LENGTH;
    ...
    for (n = 0; n < c_imtlen(tpin); ++n) {
        ...
        if (MkOutName (in_name, isuffix, trlsuffix, nsuffix, out_name,
               CHAR_LINE_LENGTH)) {
            WhichError (status);
            trlerror("Couldn't create trailer filename for %s", in_name);
            continue;
         }
