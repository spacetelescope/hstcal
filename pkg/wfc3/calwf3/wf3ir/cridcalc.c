...
**				CR_HIT->DATAREJECT, BADPIX->DETECTORPROB).
  ...
  ** of input science image. */
  ...
  for (j=jbeg; j<jend; j++) {
	...
	** them from the fit (Version 3.3) */
	...
	if (dq[wf3->ngroups-1-k] & DETECTORPROB)
	  DQSetPix (input->group[k].dq.data,i,j,
...
static void fitsamps (short nsamp, float *sci, float *err, short *dq,
  ...
  int   sample;
  int   number_CRs;             /* total number of CRs found in the set of
                                   reads */
  ...
  */
  ...
  while (nrej > 0) {
	...
	nrej=RejSpikes (tsci[idx], terr[idx], tdq[idx], diff, tcount[idx],
                   thresh, &rej);
	if (nrej > 0) {
	  dq[tlookup[idx][rej]] = dq[tlookup[idx][rej]] | DETECTORPROB;
	} else if (nrej == 0) {
	/* Only look for new CR hits if no spike was found */
	  ...
	  if (nrej > 0) {
	    ...
	    if (nrej > 0){
	      dq[tlookup[idx][0]] = dq[tlookup[idx][0]] | DETECTORPROB;
	    }
	  }
	}
...
static int RejSpikes (float *tsci, float*terr, short *dq, float *diff,
  ...
  for (k = 2; k < nsamp; k++) {
    if (((dq[k-1] & (DQIGNORE | DATAREJECT | DETECTORPROB)) == 0) &&
        ((dq[k]   & (DQIGNORE | DATAREJECT | DETECTORPROB)) == 0) &&
        ((dq[k+1] & (DQIGNORE | DATAREJECT | DETECTORPROB)) == 0)) {
      ...
      if (diff[k-2] - diff[k-1] > SPIKE_THRESH &&
	  diff[k]   - diff[k-1] > SPIKE_THRESH &&
	  diff[k-2] > 0 && diff[k] > 0) {
	  if (fabs(diff[k-1]) > max_diff) {
	      max_diff = diff[k-1];
	      (*max_samp) = k-1;
...
static int RejCRs (short *dq, float *diff, short nsamp, float thresh,
  ...
  for (k = 1; k < nsamp; k++) {
       if ((dq[k-1] & (DQIGNORE | DATAREJECT | DETECTORPROB)) == 0)
           prev_diff = diff[k-1];
       if ((dq[k] & (DQIGNORE | DATAREJECT | DETECTORPROB)) == 0) {
           /* A large positive increase in the difference is used as */
           /* a sign of a CR. */
...
static int RejFirstRead(short *dq, float *diff, short nsamp, float thresh) {
  ...
  if (((dq[0] & (DQIGNORE | DATAREJECT | DETECTORPROB)) == 0) &&
      ((dq[1] & (DQIGNORE | DATAREJECT | DETECTORPROB)) == 0)){
