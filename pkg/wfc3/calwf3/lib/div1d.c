...
# include "wf3dq.h"	/* BADFLAT = bad pixel value for divide by zero */
...
/* Divide the first SingleGroup triplet by the second, leaving the result
   ...
   2009-01-28	H.Bushouse	Changed CALIBDEFECT macro to BADFLAT, to
				coincide with WFC3 DQ assignment changes.
				(PR 61741)
*/
...
*/
	...
	for (i = 0;  i < sci_dimx;  i++) {
		...
		if (b_sci == 0.) {
		    ...
		    dqa = BADFLAT | DQPix (a->dq.data, i, line);
