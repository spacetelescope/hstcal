/* 
    ...
    output of CALWF3 trailer file comments, both to STDOUT and to
    ...
    sequences.  For the trailer files, we use the following terms:
        ...
        prefix  - string which marks the beginning of CALWF3 processing
    ...
    void SetTrlOverwriteMode (int owrite);
        - This function sets the overwrite switch.  Once set, all
            comments after the first CALWF3BEG string will be
            overwritten.
    ...
    Revised: 27 Oct 2003, H.Bushouse   Increased buffer size for trldata.
    Revised: 04 Nov 2008, H.Bushouse   Fixed bug in CloseTrlBuf that was causing an IRAF
				       segv because of a call to fclose on a NULL pointer.
				       Removed the call.
    Revised: 30 Nov 2010, H.Bushouse   Modified WriteTrlFile to check for non-null pointer to
    ...
    if (strcmp(inlist, output) != 0) {
        ...
        */
        ...
        /* We are revising/creating a single trailer file 
           We want to overwrite old CALWF3 comments with new,
...
/* This function sets the file pointer for the input file (ip) 
    ...
    21 June 2002 (HAB): Removed extraneous, bug-ridden 'tmpptr' buffer used
	during reallocation of oprefix buffer size (following CALACS changes).
*/ 
static int AppendTrlFile() {
    ...
    while ( !feof(trlbuf.fp) ) {
	    ...
	    oprefix = realloc (oprefix, (strlen(oprefix) + strlen(buf) +2));
            ...
            if (oprefix == NULL) {
                asnmessage 
			("Out of memory: Couldn't store trailer file comment.");
...
/* 
    ...
    after the first CALWF3BEG string will be overwritten.
    This will be used strictly for the RAW file trailer files,
    where the generic conversion comments should be kept, but 
    the CALWF3 comments should be overwritten with the latest results.
*/
void SetTrlOverwriteMode (int owrite) {
