...
int SinkDetect(WF3Info *wf3){
    ...
    trlmessage("Performing SINK pixel detection and flagging");
    ...
    FloatTwoDArray sinkrefab;
    FloatTwoDArray sinkrefcd;
    FloatTwoDArray sinkraz;
    ptr = openInputImage(wf3->sink.name,"SCI",1);
    if (getFloatData (ptr, &sinkrefcd))
        return(status);
    closeImage(ptr);
    
    ptr = openInputImage(wf3->sink.name,"SCI",2);
    if (getFloatData(ptr,&sinkrefab))
        return (status);
    closeImage(ptr);
    ...
    makeFloatRaz(&sinkrefcd,&sinkrefab,&sinkraz);
    ...
    freeFloatData(&sinkrefab);
    freeFloatData(&sinkrefcd);
}
