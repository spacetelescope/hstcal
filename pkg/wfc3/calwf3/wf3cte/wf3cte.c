...
except for the science team subarrays which can span quads
*/
...
int WF3cte (char *input, char *output, CCD_Switch *cte_sw,
  ...
  if (wf3.subarray) {
    ...
    if (wf3.chip == 2){ /*sci1,cd*/
        ...
        if (sci_corner[0] - ref_corner[0]  <= 0){
        ...
        */
        ...
        Sub2Full(&wf3, &subcd, &cd, 0, 1, 1);
         ...
         makeRAZ(&cd,&ab,&raw);
          ...
          /*save a copy of the full frame image for verification*/
          if (verbose) {
                Hdr junkhdr;
                initHdr(&junkhdr);
                IODescPtr out=0;
        ...
        if (verbose) {
              ...
              char tmpout[SZ_LINE+1];
              strcpy(tmpout,wf3.rootname);
              strcat(tmpout,"_bias_cd_full.fits");
              out = openOutputImage(tmpout,"",0,&junkhdr,0,0,FITSBYTE);
              putHeader(out);
              putFloatData(out,&cd.sci.data);
              closeImage(out);
              sprintf(MsgText,"full bias sub cd written to %s\n",tmpout);
              trlmessage(MsgText);
        }
      ...
      */
      if (CompareCTEParams(&subab, &cte_pars))
        ...
        if (verbose) {
              ...
              closeImage(out);
              sprintf(MsgText,"full raw ab written to %s\n",tmpout);
              trlmessage(MsgText);
        }
  ...
  if (verbose) {
        ...
        char tmpout[SZ_LINE+1];
        strcpy(tmpout,wf3.rootname);
        strcat(tmpout,"_raw_sci.fits");
        out = openOutputImage(tmpout,"",0,&junkhdr,0,0,FITSBYTE);
        putHeader(out);
        putFloatData(out,&raw.sci.data);
        closeImage(out);
        sprintf(MsgText,"full raz sci written to %s\n",tmpout);
        trlmessage(MsgText);
  }
  ...
  if (verbose) {
        ...
        char tmpout[SZ_LINE+1];
        strcpy(tmpout,wf3.rootname);
        strcat(tmpout,"_rsz_sci.fits");
        out = openOutputImage(tmpout,"",0,&junkhdr,0,0,FITSBYTE);
        putHeader(out);
        putFloatData(out,&rsz.sci.data);
        closeImage(out);
        sprintf(MsgText,"full rsz sci written to %s\n",tmpout);
        trlmessage(MsgText);
  }
  ...
  if (verbose) {
        ...
        char tmpout[SZ_LINE+1];
        strcpy(tmpout,wf3.rootname);
        strcat(tmpout,"_rsc_sci.fits");
        out = openOutputImage(tmpout,"",0,&junkhdr,0,0,FITSBYTE);
        putHeader(out);
        putFloatData(out,&rsc.sci.data);
        closeImage(out);
        sprintf(MsgText,"full rsc sci written to %s\n",tmpout);
        trlmessage(MsgText);
  }
  ...
  if (verbose) {
        ...
        char tmpout[SZ_LINE+1];
        strcpy(tmpout,wf3.rootname);
        strcat(tmpout,"_chg_sci.fits");
        out = openOutputImage(tmpout,"",0,&junkhdr,0,0,FITSBYTE);
        putHeader(out);
        putFloatData(out,&chg.sci.data);
        closeImage(out);
        sprintf(MsgText,"full chg sci written to %s\n",tmpout);
        trlmessage(MsgText);
  }
  ...
  if (verbose) {
        ...
        char tmpout[SZ_LINE+1];
        strcpy(tmpout,wf3.rootname);
        strcat(tmpout,"_rzc_sci.fits");
        out = openOutputImage(tmpout,"",0,&junkhdr,0,0,FITSBYTE);
        putHeader(out);
        putFloatData(out,&rzc.sci.data);
        closeImage(out);
        sprintf(MsgText,"full rzc sci written to %s\n",tmpout);
        trlmessage(MsgText);
  }
...
int raw2raz(WF3Info *wf3, SingleGroup *cd, SingleGroup *ab, SingleGroup *raz){
 ...
 if (wf3->subarray){
   ...
   for (k=0;k<4;k++){
     sprintf(MsgText,"k[%i] for pre scan: %f\n",k,bias_pre[k]);
     trlmessage(MsgText);
     for (i=0; i<subcol;i++){
       for (j=0;j<RAZ_ROWS; j++){
      ...
      /*SUBTRACT THE BIAS CALCULATED ABOVE, SPANNING THE QUADS WITH K
        ...
        has prescan bias pixels so this is done above.

      */
      for (k=0;k<4;k++){
        sprintf(MsgText,"k[%i] for post scan: %f\n",k,bias_post[k]);
        trlmessage(MsgText);
        for (i=0; i<subcol;i++){
          for (j=0;j<RAZ_ROWS; j++){
  ...
  /* convert array to electrons, multiply by the gain, ignore mask */
  for (i=0; i<RAZ_COLS;i++){
    for (j=0;j<RAZ_ROWS; j++){
      if (Pix(raz->dq.data,i,j)){
        Pix(raz->sci.data,i,j) *=  gain;
      }
    }
  }
...
int findPostScanBias(SingleGroup *raz, float *mean, float *sigma){
  ...
  */
  ...
  for (k=0;k<4;k++){  /*for each quadrant cdab = 0123*/
    ...
    sprintf(MsgText,"npix(%i) = %i",k+1,npix);
    trlmessage(MsgText);
    if (npix > 0 ){
...
the prescan is not used as part of the correction.
...
*/
    ...
    */
    ...
    for(NIT=1; NIT<=100; NIT++){
       ...
       /*This is probably a large time sink because the arrays are being
