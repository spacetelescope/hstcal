...
/* This routine gets the value of a reference file keyword from the
   ...
   Howard Bushouse, 23 Aug 2000:
	Function edited to work with WFC3.
...
*/
	...
	if ((filename = calloc (SZ_FITS_REC+1, sizeof(char))) == NULL)
	    return (status = OUT_OF_MEMORY);

	if (GetKeyStr (phdr, keyword, USE_DEFAULT, "", filename, SZ_FITS_REC))
	    return (status);
